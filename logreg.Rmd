...
#--------------------------------------------------------------------
for (i in 1:10){
  image(train.X[,,i], col = gray((0:255/255)))
}
```
```{r}
summary(train.X)
...
#   Ridge Regression
#--------------------------------------------------------------------
ridge.fit <- glmnet(tst$X, tst$y, family="binomial", alpha = 0)
...
#   Ridge Regression
#--------------------------------------------------------------------
lasso.fit <- glmnet(tst$X, tst$y, family="binomial", alpha = 1)
#print (lasso.fit)
lasso.pred <- predict(lasso.fit, trn$X, type="response")
print(data.frame(pred=as.integer(lasso.pred > 0.5), true=trn$y) %>% count(true,pred) %>% mutate(freq=n/sum(n)))
plot(lasso.fit)
