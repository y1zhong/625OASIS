...
#cbind(cv.fit$lambda, cv.fit$cvm)
...
confusion.glmnet(ridge.fit, newx = tst$X, newy = tst$y)
plot(ridge.fit)
...
#   Cross Validation Ridge
#--------------------------------------------------------------------
cv.fit2 = cv.glmnet(combined$X, combined$y, family = "binomial", 
                   alpha = 1, 
                   standardize = FALSE, keep = TRUE)
#cbind(cv.fit$lambda, cv.fit$cvm)
best_param2 = cv.fit2$lambda.min   #best parameter
best_coef2 = coef(cv.fit2, s = "lambda.min")
...
lasso.fit <- glmnet(trn$X, trn$y, family="binomial", alpha = 1, lambda = best_param2)
#print (lasso.fit)
...
#--------------------------------------------------------------------
idmin = match(cv.fit$lambda.min, cv.fit$lambda)
idmin2 = match(cv.fit2$lambda.min, cv.fit2$lambda)


plot(roc.glmnet(cv.fit$fit.preval, newy = combined$y)[[idmin]])
plot(roc.glmnet(cv.fit2$fit.preval, newy = combined$y)[[idmin2]])

confusion.glmnet(ridge.fit, newx = tst$X, newy = tst$y, s = 0.1)
confusion.glmnet(lasso.fit, newx = tst$X, newy = tst$y, s = 0.01)
