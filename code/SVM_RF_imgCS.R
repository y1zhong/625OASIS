...
ctrl <- trainControl(method = "cv", number = 10, savePredictions = TRUE, 
                     classProbs = TRUE, allowParallel = T,verboseIter = T) 

## LDA classification
library(MASS)
set.seed(123)
lda.fit <-train(x = Training_data[,-1], y = Training_data$y, 
                data = Training_data, method = "sparseLDA",
...
lda.res = list(lda.fit=lda.fit,lda.TrainAcc=lda.TrainAcc,
               lda.TestAcc=lda.TestAcc,lda.cfmat=lda.cfmat,lda.FPFNSeSp=lda.FPFNSeSp)
lda.res
...
set.seed(123)
svm.rad.fit <-train(y~., data = Training_data, method = "svmRadial",
                    trControl = ctrl,tuneLength=10)
...
nb.res = list(nb.fit = nb.fit,nb.TrainAcc = nb.TrainAcc,
              nb.TestAcc = nb.TestAcc, nb.cfmat = nb.cfmat,
              nb.FPFNSeSp = nb.FPFNSeSp)
nb.res
...
CDR_acc = data.frame(
  ...
  TrainAccuracy = c(lda.TrainAcc, svm.lin.TrainAcc, svm.rad.TrainAcc, 
                    rf.TrainAcc,nb.TrainAcc),
  TestAccuracy = c(lda.TestAcc, svm.lin.TestAcc, svm.rad.TestAcc, 
                   rf.TestAcc, nb.TestAcc)
)
knitr::kable(CDR_acc)

pred_and_true = list(True_cdr = True_cdr,
                     lda.pred = as.numeric(lda.pred) - 1, 
                     svm.lin.pred = as.numeric(svm.lin.pred) - 1, 
                     svm.rad.pred = as.numeric(svm.rad.pred) - 1,
                     rf.pred = as.numeric(rf.pred) - 1,
                     nb.pred=as.numeric(nb.pred) - 1)

SVM_RF_ImgCS_result = list(lda.res = lda.res, 
                           svm.lin.res = svm.lin.res,
                           svm.rad.res = svm.rad.res,
                           rf.res = rf.res,
                           nb.res = nb.res,
                           CDR_acc = CDR_acc,
