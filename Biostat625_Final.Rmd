...
**e) CNN**   
To avoid information loss from 3-d images to 2-d matrix in above methods , we tried to employ convolutional neural network, which is a popular deep learning technique in medical imaging analysis. VGG is a deep learning architecture shown to be able to reach top accuracy in image classification tasks. It utilizes 3-d filters and fully connected layers that usually need hundreds of millions of parameters for a successfully trained model. In this project, we mimicked VGG16 with 16 layers, 1 millions parameters. We used Keras library in Python script and GPU in Google Colab to efficiently train our model. However, we did not find a meaningful train feature that has the discernibility to classify test data into correct diagnosis results. With several modified architectures, all the accuracies are around or below 50%. We did not report the evaluation here and left it for future exploration. 
