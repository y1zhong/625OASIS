...
**e) CNN**   
To avoid information loss from 3-d images to 2-d matrix in above methods , we tried to employ convolutional neural network, which is a popular deep learning technique in medical imaging analysis. Visual Geometry Group(VGG) is a deep learning architecture shown to be able to reach top accuracy in image classification tasks.[3] It utilizes 3-d filters and fully connected layers that usually need hundreds of millions of parameters for a successfully trained model. In this project, we mimicked VGG16 structure with around 16 layers and 50k parameters. We used Keras library in Python script and GPU in Google Colab to efficiently train our model. However, we did not find a meaningful train feature that has the discernibility to classify test data into correct diagnosis results. Even with several modified architectures including more than one million parameters, all the accuracies are around or below 50%. We did not report the evaluation here and left it for future exploration. 

# Deliverable and Code Repository
### Shiny App
Shiny Application We built our Shiny application using the ridge LR as our classifier embedded behind the dashboard. This means that the dashboard can take patients MRI data and some demographic and clinical input to predicts of Dementia diagnosis using CNN model. The Shiny application can be viewed by accessing through 'shinyapps.io', https://j3z4te-tiankai-xie.shinyapps.io/shiny/, and a workable input called myMRI.zip could be found in shiny folder. Packages like Shiny and oro.nifti must be installed before this app can be run.
...
```{r, echo=FALSE,fig.cap="ROC curves of selected models", fig.align='center', out.width='60%', out.height='40%'}
library(knitr)
library(kableExtra)
# library(kableExtra)
load("./tables and figures/result_table.RData")
kbl(result_table,"latex", booktabs = T, align="c", col.names = NULL, caption = "Table 2. Classification results of sensitivity, specificity, and accuracy") %>%
  kable_styling( full_width = F, position = "left") %>%
  add_header_above(c("","Image only"=6, "Image+CS"=5))
knitr::include_graphics("./tables and figures/ROC.png")
```
